...
class FirstChoiceProcessor(ColesGroupProcessor):
    """
    First Choice Liquor-specific processor implementing the first principles scraping strategy.
    """
    def __init__(self):
        super().__init__("firstchoiceliquor")
        self._saved_html_dir = os.path.join(os.path.dirname(__file__), "debug")

    def fetch_url(self, url: str) -> Optional[str]:
        """
        Override fetch_url with multi-strategy approach for First Choice.
        Strategy: Desktop Chrome + Stealth Proxy + JS + Wait for element.
        """
        result = self._fetch(url, render_js=True, premium_proxy=True, stealth_proxy=True,
                            wait="10000", wait_for=".product-tile-list", custom_headers={
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"
        })
    ...
    def discover_tasks(self, url: str) -> List[dict]:
        """
        Discovery for First Choice using HTML parsing to extract the page count.
        """
        content = self.fetch_url(url)
        if not content:
            return [{"url": url, "metadata": {"page": 1}}]
        
        self._save_html(url, content)
        ...
        if result_count_elem:
            ...
            if match:
                total_results = int(match.group(1))
                num_pages = math.ceil(total_results / FIRSTCHOICE_MAX_RESULTS_PER_PAGE)
                return [{"url": url + f"?page={p}", "metadata": {"page": p}} for p in range(1, num_pages + 1)]
    ...
    def get_items(self, url: str, metadata: Optional[dict] = None) -> Tuple[List[Item], Optional[dict]]:
        """
        Extract items from First Choice using direct HTML extraction.
        """
        result = []
        ...
        for tile in product_tiles:
            try:
                ...
                if thumbnail:
                    link = thumbnail.get("href", "")
                    if link and not link.startswith("http"):
                        link = f"https://www.firstchoiceliquor.com.au{link}"
                ...
                if thumbnail:
                    ...
                    if img_elem:
                        image = img_elem.get("src", "")
                        if image and not image.startswith("http"):
                            image = f"https://www.firstchoiceliquor.com.au{image}"
                ...
                print(f"Error parsing First Choice HTML product: {e}")
