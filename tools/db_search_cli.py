...
def main():
    ...
    parser.add_argument('--sort', choices=['score', 'price', 'percent', 'ml'], default='score',
                        help='Category to sort by (default: score)')
    ...
    try:
        ...
        print("-" * 100)
        header = f"{'Store':<10} | {'Brand':<20} | {'Name':<40} | {'Price':<8} | {'$/Std':<10}"
        print(header)
        print("-" * 100)

        for i, row in enumerate(results[:args.limit]):
            # Row mapping based on table schema in create_connection:
            # 0:ID, 1:store, 2:brand, 3:name, 4:type, 5:price, 6:link, 7:pack_qty, 8:ml, 9:percent, 10:stdDrinks, 11:score, 12:image, 13:shortimage
            store = str(row[1])
            brand = str(row[2])
            name = str(row[3])
            price = f"${row[5]:.2f}"
            score_value = row[11]
            score_repr = f"${score_value:.2f}" if score_value is not None else "n/a"
            
            # Truncate long strings for display
            brand = (brand[:17] + '...') if len(brand) > 20 else brand
            name = (name[:37] + '...') if len(name) > 40 else name
            
            print(f"{store:<10} | {brand:<20} | {name:<40} | {price:<8} | {score_repr:<10}")
